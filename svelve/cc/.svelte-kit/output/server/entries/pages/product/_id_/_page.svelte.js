import { f as attr_style, h as stringify, b as attr_class, c as escape_html, a as attr, i as derived, j as head, e as ensure_array_like } from "../../../../chunks/index2.js";
function PriceChart($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    let { data, height = 300 } = $$props;
    $$renderer2.push(`<div class="chart-container svelte-afumdx"${attr_style(`height: ${stringify(
      // Already in kroner
      height
    )}px`)}>`);
    if (data.length > 0) {
...
function StorePriceCard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const discountPercent = derived(() => hasDiscount() && originalPrice ? Math.round((1 - price / originalPrice) * 100) : 0);
    const formatPrice = (p) => {
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    const savings = derived(() => data.highestPrice && data.lowestPrice ? data.highestPrice - data.lowestPrice : 0);
    const savingsPercent = derived(() => data.highestPrice ? Math.round(savings() / data.highestPrice * 100) : 0);
    let selectedImage = 0;
